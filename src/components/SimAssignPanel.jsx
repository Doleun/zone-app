...
export default function SimAssignPanel({
  ...
  /* undefined → null 변환 (Firestore 저장 전) */
  const sanitize = (drivers) =>
    drivers.map(d => Object.fromEntries(
      Object.entries(d).map(([k, v]) => [k, v === undefined ? null : v])
    ));

  const saveDrivers = useCallback(async (newDrivers) => {
    await onSaveDrivers(sim.id, sanitize(newDrivers));
