...
/* ══════════════════════════════════════
   RegionCampPanel (master 전용)
   - 지역 추가/삭제
   - 캠프 추가/삭제 (지역 선택 후)
══════════════════════════════════════ */
export default function RegionCampPanel({ regions, camps, showToast }) {
  ...
  const handleDeleteRegion = async (id) => {
    ...
    if (!window.confirm(`"${r?.name}" 지역을 삭제할까요?\n해당 지역 캠프도 모두 삭제됩니다.`)) return;
    try {
  ...
  const handleDeleteCamp = async (id) => {
    ...
    if (!window.confirm(`"${c?.name}" 캠프를 삭제할까요?`)) return;
    try {
      await deleteCamp(id);
      showToast('✅ 삭제 완료');
    } catch(e) { showToast('❌ 실패: ' + e.message); }
